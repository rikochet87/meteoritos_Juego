...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action("recarga_escudo"):
		naver_player.get_escudo().controlar_energia(radio_energia_entregada)
	elif event.is_action("recarga_laser"):
		naver_player.get_laser().controlar_energia(radio_energia_entregada)	
